...
const OutputTable = ({ results }) => {
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold mb-4">Process Results</h2>
          ...
          <tbody>
            <AnimatePresence>
              {processes.map((p) => (
                ...
                >
                  <td className="p-2 font-semibold text-blue-600">{p.pid}</td>
      ...
      <div className="grid grid-cols-2 gap-4 mt-4">
        ...
        >
          ...
          <p className="text-xl font-bold">{avgWaiting}</p>
        </motion.div>

        <motion.div
          initial={{ scale: 0.9, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          className="p-4 bg-yellow-100 rounded-xl shadow"
        >
          <p className="text-gray-600 text-sm">Avg Turnaround Time</p>
          <p className="text-xl font-bold">{avgTurnaround}</p>
        </motion.div>
      </div>
    </div>
  );
